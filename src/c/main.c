...
static void developingforsquare(Layer * layer, GContext * ctx){
  ...
  GRect hour_rect=grect_centered_from_polar(GRect(bounds.origin.x+bounds.size.w/2, bounds.origin.y+bounds.size.h/2, 0, 0),
...
static void developingforround(Layer * layer, GContext * ctx){
  ...
  GRect iconstep=GRect(temprect.origin.x, loc_rect.origin.y, 20, loc_rect.size.h);
  //Alt location
  GRect loc_rect_alt=GRect(loc_rect.origin.x+5, temprect.origin.y, loc_rect.size.w-10, loc_rect.size.h+10);
  GRect temprect_alt=GRect(temprect.origin.x, hour_rect.origin.y-temprect.size.h+5, temprect.size.w, temprect.size.h);
  GRect condrect_alt=GRect(condrect.origin.x, temprect_alt.origin.y, condrect.size.w, condrect.size.h);
  GRect iconstep_alt=GRect(iconstep.origin.x, loc_rect_alt.origin.y, 20, loc_rect_alt.size.h);
  GRect numstep=GRect(iconstep.origin.x+iconstep.size.w, iconstep.origin.y, loc_rect_alt.size.w-iconstep_alt.size.w, loc_rect.size.h);
  GRect numstep_alt=GRect(iconstep_alt.origin.x+iconstep_alt.size.w, iconstep_alt.origin.y, loc_rect_alt.size.w-iconstep_alt.size.w, loc_rect_alt.size.h);
  if (settings.TopComp>=0 || settings.BottomComp>=0){
    if (!settings.BTOn){
      graphics_draw_text(ctx, "a", FontSymbol, loc_rect, GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
    }
